...
export class ParcoursViewer {
  ...
  buildBreadcrumb(slide) {
    ...
    // Fallback avec escapeHtml inline
    const esc = (s) => typeof s !== 'string' ? '' : s
      .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
      .replace(/"/g, '&quot;').replace(/'/g, '&#039;');

    if (!this.epic || !slide) { return ''; }
    const parts = [`<span class="pv-breadcrumb-item">${esc(this.epic.title)}</span>`];
    for (const sec of (slide.path || [])) {
      parts.push('<span class="pv-breadcrumb-sep">›</span>');
      parts.push(`<span class="pv-breadcrumb-item">${esc(sec.title)}</span>`);
    }
    parts.push('<span class="pv-breadcrumb-sep">›</span>');
    parts.push(`<span class="pv-breadcrumb-item current">${esc(slide.title)}</span>`);
    return parts.join('');
  }
    ...
    for (const item of structure) {
      if (item.type === 'section') {
        html += `<li role="treeitem" aria-expanded="true" class="pv-menu-section">
          ...
          <span class="pv-menu-label">${esc(item.title)}</span></button>
        ...
        html += `<li role="treeitem" class="pv-menu-slide ${state} ${opt}" data-slide-id="${item.id}">
          ...
          <span class="pv-menu-label">${esc(item.title)}</span>${optTag}</button></li>`;
      }
    }
    return `${html}</ul>`;
  }
